...
- Add Local Services as a bundled provider for local ports, HTTP endpoints, processes, and development runtime checks.
...
- Ship Mac System as a built-in provider resource instead of requiring a separate GitHub install for new users.
...
- Require provider manifests to declare an icon and render pinned provider navigation as compact icon-only tabs.
...
- Keep the main popover focused on Overview plus pinned provider pages.
- Add per-provider first-level display pinning from the installed-provider list and overview rows.
- Reorder plugin management by usage frequency: Installed, Marketplace, GitHub.
